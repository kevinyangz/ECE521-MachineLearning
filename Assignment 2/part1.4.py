...
def CalculateWoptimal():
    ...
    return X,y_target,W_optimal,meanSquaredError,y_predicted
def calculateMSE():
    y_target = tf.placeholder(tf.float32, [None,1], name='target_y')
    X = tf.placeholder(tf.float32, [None, 785], name='input_x') 
...
x_data,y_data,wnormal,mse,y_predicted=CalculateWoptimal()
...
Train_meanSquaredError,wNormalCal,ytrainresult=sess.run([mse,wnormal,y_predicted],feed_dict={x_data:trainData, y_data: trainTarget})
...
for i in range(np.shape(trainTarget)[0]):
    accuracy_list.append(trainTarget[i]==(ytrainresult[i]>0.5))
accuracy = accuracy_list.count(True) / len(ytrainresult) * 100
print("Accuracy for the train set is %s%s,MeanSquarederror is %s"%(accuracy,'%',Train_meanSquaredError))
...
print("Accuracy for the testData set is %s%s,MeanSquarederror is %s"%(accuracy,'%',test_meanSquaredError))
